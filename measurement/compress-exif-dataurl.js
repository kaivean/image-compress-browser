...
define(function (require, exports) {
    ...
    function getOrientation(file, callback) {
        ...
        fileReader.onload = function(e) {
            stat.orient = performance.now();
            var data = exif.readFromBinaryFile(e.target.result);
            stat.orient = performance.now() - stat.orient;
            $('#other').append('get Orientation by exif ' + data['Orientation']);
    ...
    exports = function (file, callback) {
        ...
        loadFileToImg(file, function (img) {
            ...
            getOrientation(file, function (orientation) {
                ...
                canvasHandle(img, orientation, function (dataUrl, width, height) {
                    var becomeLarge = img.src.size <= dataUrl.size;
                    if (becomeLarge) {
                        dataUrl = img.src;
                    }
                    stat.newSize = dataUrl.length;
